        ...
        <table id="example1" class=" table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><img src="{{ asset('uploads/imageDetail/'.$image->url)}}" alt="Image Current" width="50px"
                            height="50px"></td>
                    <td> @if($image->lesson)
                        {{ $image->lesson->title}}
                        @endif
                    </td>
                    <td>{{ date('d/m/y - H:i:s',strtotime($image->created_at))}}
                    </td>
                    <td>
                        {{ date('d/m/y - H:i:s ', strtotime($image->updated_at))}}
                    </td>
                    <!-- <td> <a href="{{ route('admin.image.edit', ['id' => $image->id]) }}">
                            Edit </a>
                    </td>
                    <td> <a onclick="return confirmationDelete('image')"
                            href="{{route('admin.image.destroy', ['id'=> $image->id])}}"> Delete </a> </td> -->
                </tr>
                @endforeach
            </tbody>
            <tfoot>
